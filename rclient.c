...
SSL_CTX* InitCTX(void)
{
    ...
    int temp = SSL_CTX_load_verify_locations(ctx, "OpenSSL/rogue_ca.crt",NULL);
    if (temp == 0) {
        ERR_print_errors_fp(stderr);
        abort();
    }
...
int main(int argc, char *argv[])
{
     ...
     */
    LoadCertificates(ctx, "OpenSSL/rogue_client.crt","OpenSSL/rogue_client.key");
